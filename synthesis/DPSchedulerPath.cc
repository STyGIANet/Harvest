...
double DPScheduler::getDemandStep(int i){
  ...
  if (collective_ == "direct-all-to-all" || collective_ == "all-to-all-nd"){
...
std::pair<Topology, double> DPScheduler::completion_time_all_to_all(int a, int b) {
  std::pair<int,int> key{a,b};
  auto itc = cache_.find(key);
  if (itc != cache_.end()) return itc->second;
  ...
  for (int shift = 0; shift < numShit; ++shift) {
    ...
    Topology tempR;
    tempR.reserve((size_t)n_ * (size_t)(n_ - 1));
    for (int u = 0; u < n_; ++u) {
      for (int v = 0; v < n_; ++v) {
        if (u == v) continue;
        tempR[Edge{u,v}] = 0;
    ...
    Topology toUseR = baseRing;
    for (const auto& kv : toUseR) {
      const Edge& e = kv.first;
      int val = kv.second;
      int uprime = e.u % n_;
      int vprime = (e.v + shift) % n_;
      if (uprime == vprime) continue;
      tempR[Edge{uprime, vprime}] = val;
    }
    bool redundantR = false;
    for (const auto& prev : y) {
      if (topoEqualFull(tempR, prev)) {
        redundantR = true;
        break;
      }
    }
    if (!redundantR) {
      y.push_back(std::move(tempR));
      topoSpace++;
    }
  }
  ...
  for (int search = 0; search < topoSpace; ++search) {
    ...
    try {
      ...
      for (const auto& kv : demandST) {
        Pair st = kv.first;
        double dint = kv.second;
        ...
        for (int i = a; i <= b; ++i) {
          for (int pidx = 0; pidx < (int)paths.size(); ++pidx) {
            PathKey k{i, st.first, st.second, pidx};
            auto it = g.find(k);
            if (it != g.end())
              sum += it->second;
          }
        ...
        model.addConstr(sum == theta[(size_t)a] * dint);
      }

      // std::cout << "adding capacity constraints\n";

      // Convert capacity constraints below per-step, to combined over (a,b)
      // Build edge incidence just once for the whole (a,b)
      std::unordered_map<Edge, std::vector<PathKey>, EdgeHash> edgeInc;
      edgeInc.reserve(edges.size() * 4);
      ...
      for (int i = a; i <= b; ++i) {
        for (const auto& dmd : steps_[(size_t)i - 1]) {
          ...
          for (int pidx = 0; pidx < (int)paths.size(); ++pidx) {
            const Path& path = paths[(size_t)pidx];
            for (const auto& e : path) {
              edgeInc[e].push_back(PathKey{i,s,t,pidx});
            }
          }
        }
      }
      ...
      // for (int i = a; i <= b; ++i) {
        double bits = 0;
        for (int i = a; i <= b; ++i) {
          bits += getDemandStep(i);
        }
        
        // model.addQConstr(2 * theta[(size_t)i] * T[(size_t)i] == 2 * beta_ * bits);
        GRBVar th = theta[(size_t)a];
        GRBVar Ti = T[(size_t)a];
      ...
      // for (int i = a; i <= b; ++i) {
        ...
        // Since scale is 1 anyway otherwise, getting rid of if else
        // T[(size_t)i] / (beta_ * bits * SCALE/ d_) this term is theta and unitless
        obj += alpha_*SCALE + T[(size_t)a] + delta_ * ( T[(size_t)a] / (beta_ * bits / d_));
      // }
      model.setObjective(obj, GRB_MINIMIZE);
