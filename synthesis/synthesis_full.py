    ...
    def completion_time(self, a: int, b: int) -> Tuple[Topology, float]:
        ...
        for u in range(self.n):
            for v in range(self.n):
                ...
                if self.relaxation == 1:
                    x[u, v] = model.addVar(vtype=GRB.CONTINUOUS, lb=0, ub=1, name=f"x[{u},{v}]")
                    z[u, v] = model.addVar(vtype=GRB.CONTINUOUS,lb=0)
                    
                    # Piece wise
                    xvar = x[u,v]
                    zvar = z[u,v]
                    K = 10
                    xs = [k/(K-1) for k in range(K)]
                    ys = [t - t*t for t in xs]
                    model.addGenConstrPWL(xvar, zvar, xs, ys, name=f"pwl_{u}_{v}")

                    # model.addQConstr(z[u,v] >= x[u,v] - x[u,v]*x[u,v])

                    # model.addConstr(z[u, v] >= 1 - x[u, v])

                    # model.addConstr(z[u, v] >= x[u,v]-0.5)
                    # model.addConstr(z[u, v] >= x[u,v]-x[u,v]*x[u,v])
                else:
                    x[u, v] = model.addVar(vtype=GRB.INTEGER, lb=0, name=f"x[{u},{v}]")
                    # Won't be used, just for compatability
                    z[u, v] = model.addVar(vtype=GRB.CONTINUOUS,lb=0)
        ...
        if self.relaxation == 1:
            model.setObjective(gp.quicksum(1e6*(z[u,v]) for u in range(self.n) for v in range(self.n) if u!=v)+gp.quicksum(self.alpha+T[i]*(1+ self.delta/(self.beta * self.chunksizes[i-1])) for i in range(a, b + 1)), GRB.MINIMIZE)
        else:
        ...
        # for u in range(self.n):
        #     print(sum([x[u,v].X for v in range(self.n) if u!=v]))
        # print()
        topo: Topology = {}
        for (u, v), var in x.items():
            ...
            if val - math.floor(val) > 0.5:
                ...
                print(u,v,val,z[u,v].X)

        if topo=={}:
            print("###################")
            print(model.SolCount)
            print()
            print(key,model.ObjVal)
            assert(False)
        cost = model.ObjVal
        self._cache[key] = (topo, cost)
        return topo, cost
